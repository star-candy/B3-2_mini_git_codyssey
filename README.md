...
### 커스텀 정렬 알고리즘 (Insertion Sort)
- 표준 API를 금지하는 제약사항에 따라 삽입 정렬(Insertion Sort)을 직접 구현
- ![alt text](images/img.gif)
- arr을 정렬된 영역과 비정렬된 영역으로 나눔
    - 비정렬된 영역의 첫 번째 원소를 정렬된 영역의 올바른 위치에 삽입하는 방식
    - 정렬된 영역: arr[0 ~ i-1], 비정렬된 영역: arr[i ~ n-1]
